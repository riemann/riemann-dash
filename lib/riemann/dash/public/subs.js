var subs = (function() {
  ...
  var Subscription = Backbone.Model.extend({
    ...
    initialize: function(id, query, f, rows_str, cols_str) {
      this.id = id;
      this.query = query;
      this.f = f;
      this.rows_str = rows_str;
      ...
      ws.onmessage = _.bind(function(e) {
        ...
        if (active) {
          ...
          if (event.state !== "expired") { // only expired events have no TTL
            ...
            // events.
            dummy = {};
            dummy[this.rows_str] = event[this.rows_str];
            dummy[this.cols_str] = event[this.cols_str];
  ...
  // opened immediately.
  var subscribe = function(query, f, r = 'host', c = 'service') {
    var sub = new Subscription(newId(), query, f, r, c).open();
    subs[sub.id] = sub;
