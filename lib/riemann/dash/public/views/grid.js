(function() {
  ...
  var Grid = function(json) {
    ...
    this.row_link = json.row_link || 'nil';
    this.col_link = json.col_link || 'nil';
  ...
  Grid.prototype.json = function() {
    return $.extend(view.View.prototype.json.call(this), {
      ...
      row_link: this.row_link,
      col_link: this.col_link
    });
  };
  ...
  Grid.prototype.render = function() {
    ...
    var col_link = this.col_link;
    this.cols.forEach(function(name, i) {
      var element = $('<th><a/>');
      if(col_link == 'nil') {
        element.text(name || 'nil');
      } else {
        link = element.find('a');
        link.attr("href", col_link.replace('%{col_name}', name));
        link.text(shortColNames[i] || 'nil');
      }
      row.append(element);
    });
    ...
    this.rows.forEach(function(rowName, i) {
      var row = $("<tr><th><a/></th>");
      if (this.row_link == 'nil') {
        row.find('th').text(shortRowNames[i] || 'nil');
      } else {
        link = row.find('a');
        link.attr("href", this.row_link.replace('%{row_name}', rowName));
        link.text(shortRowNames[i] || 'nil')
      }
