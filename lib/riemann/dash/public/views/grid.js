(function() {
  ...
  Grid.prototype.editForm = function() {
    return Mustache.render('<label for="title">Title</label>' +
        ...
        '<label for="orient">Layout</label>' +
        '<input type="radio" name="orient" value="hosts" checked="true">by Host</input>' +
        '<input type="radio" name="orient" value="service" checked="true">by Service</input>'
  ...
  Grid.prototype.partialRender = function(event) {
    ...
    var rowIndex = (orient == "host")? hostIndex : serviceIndex;
    var colIndex = (orient == "host")? serviceIndex : hostIndex;
    var row = this.el.find('tbody tr')[rowIndex];
    var td = $($(row).find('td')[colIndex]);
